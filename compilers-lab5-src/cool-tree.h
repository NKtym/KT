...
public:
  std::string id_name;
  void visit(object_class &ref) override { id_name = ref.name->get_string(); }
  std::string let_id;
  Expression let_init = nullptr, let_body = nullptr;
  void visit(let_class &ref) override {
    let_id = ref.identifier->get_string();
    let_init = ref.init;
    let_body = ref.body;
  }
  Expressions block_body = nullptr;
  void visit(block_class &ref) override { block_body = ref.body; }
  Expression recv = nullptr;
  Symbol disp_name = nullptr;
  Expressions args = nullptr;
  void visit(dispatch_class &ref) override {
    recv = ref.expr;
    disp_name = ref.name;
    args = ref.actual;
  }
  Expression plus_e1 = nullptr, plus_e2 = nullptr;
  void visit(plus_class &ref) override {
    plus_e1 = ref.e1;
    plus_e2 = ref.e2;
  }
  Expression method_body = nullptr;
  void visit(method_class &ref) override { method_body = ref.expr; }
};
class NewVisitor : public Visitor {
public:
  Symbol type_name;
  void visit(new__class &n) override { type_name = n.type_name; }
};
